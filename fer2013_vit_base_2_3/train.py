...
class DynamicWeightAdjuster:
    ...
    def update_weights(self, class_accuracies, epoch):
        ...
        for i in range(len(class_accuracies)):
            ...
            # 特别关注fear(2)和sad(5)类别
            if i == 2 or i == 5:  # fear 和 sad
                if current_acc < 0.6:  # 低于60%
                    if (current_acc - recent_avg) < -0.02:  # 下降
                        new_weights[i] *= 2.2  # 大幅增加权重
                        print(f"    ⚡ 大幅增加 {['angry', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise'][i]} 权重")
                    elif abs(current_acc - recent_avg) < 0.015:  # 停滞
                        new_weights[i] *= 1.7  # 中等增加权重
                        print(f"    ⚡ 增加 {['angry', 'disgust', 'fear', 'happy', 'neutral', 'sad', 'surprise'][i]} 权重")
                elif current_acc > 0.7:  # 表现好，降低权重
                    new_weights[i] *= 0.9
...
def train(config):
    ...
    # 调整优化器参数
    if config.start_epoch > 20:  # 20轮后降低weight_decay
        optimizer.param_groups[0]['weight_decay'] = 0.01
        print(f"  ⚖️  weight_decay调整: 0.05 → 0.01")
    # ==================== 优化器微调结束 ====================
