...
def get_vit_lr_groups(model, base_lr, weight_decay, layer_decay=0.90):
    """
    1. 自动识别新模块并给予 1.5x LR。
    2. 将 Layer Decay 设为 0.90，保持 Backbone 的适配活性。
    """
    num_layers = 12 + 1 
    param_groups = {}
    
    print("\n" + "="*50)
    print("LR Grouping Analysis:")
    
    for name, param in model.named_parameters():
        if not param.requires_grad: continue
        this_wd = 0. if (param.ndim <= 1 or name.endswith(".bias")) else weight_decay
        
        # 核心探测逻辑：包含 head, spatial, synergistic 关键词的层获得补贴
...
def main():
    # --- 基础配置 ---
    SAVE_DIR = './exp5_refined_final'
    os.makedirs(SAVE_DIR, exist_ok=True)
    LOG_FILE = f"{SAVE_DIR}/train_log.csv"
    
    BASE_LR = 2e-5      
    LAYER_DECAY = 0.90  # 释放底层活性
    WEIGHT_DECAY = 0.05
    EPOCHS = 80
    WARMUP_EPOCHS = 5   # 线性预热，防止开局由于随机权重导致的抖动
    ...
    # 策略：前 5 轮 Warmup，后 75 轮 Cosine 退火
    ...
    for epoch in range(EPOCHS):
        ...
        pbar = tqdm(train_loader, desc=f"Exp5-Refined E{epoch+1}")
        for imgs, lbls in pbar:
            imgs, lbls = imgs.cuda(), lbls.cuda()
            optimizer.zero_grad(set_to_none=True) # 3090 显存加速
