...
def train_one_fold(fold, train_idx, val_idx, full_dataset, fer_weights):
    ...
    cw = compute_class_weight('balanced', classes=unique_classes, y=fold_targets)
    
    # 映射类别权重
    weight_dict = {cls: w for cls, w in zip(unique_classes, cw)}
    sampler = WeightedRandomSampler([weight_dict[t] for t in fold_targets], len(fold_targets))
    ...
    for epoch in range(EPOCHS):
        ...
        # 日志计算 (完全对标 Exp5 CSV 字段)
        t_loss = train_loss / len(train_idx)
        t_acc = train_corrects.double().item() / len(train_idx)
        ...
        if v_acc > best_acc:
            ...
            with open(f"{SAVE_DIR}/best_report.txt", 'w') as f:
                f.write(classification_report(targets, preds, digits=4, zero_division=0))
            
        print(f"Fold {fold+1} E{epoch+1} | Acc: {v_acc:.4f} | F1: {f1:.4f} | LR: {current_lr:.2e}")

    return best_acc

def main():
    DATA_ROOT = './ck/images' # 确认该目录下直接是 7 个文件夹
    FER_WEIGHTS = './exp5_refined_v3/run_8/best.pth'
    
    full_dataset = datasets.ImageFolder(DATA_ROOT)
    
    # 核心：使用 StratifiedKFold 保证类别比例一致
