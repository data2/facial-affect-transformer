...
def train_epoch(model, train_loader, criterion, optimizer, scheduler,
    ...
    # 确保aug_type在统计字典中（安全处理）
        ...
        if (batch_idx + 1) % 10 == 0:
            ...
            cutmix_pct = f"{aug_stats.get('cutmix', 0)/total_batches*100:.1f}%"
            mixup_pct = f"{aug_stats.get('mixup', 0)/total_batches*100:.1f}%"
            medium_pct = f"{aug_stats.get('medium', 0)/total_batches*100:.1f}%"

            progress_bar.set_postfix({
                'Loss': f'{avg_loss:.4f}',
                'Acc': f'{accuracy:.2f}%',
                'LR': f'{optimizer.param_groups[0]["lr"]:.2e}',
                'Aug': f"{cutmix_pct}/{mixup_pct}/{medium_pct}"
            })
